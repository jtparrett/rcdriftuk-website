// Proper 2D car drifting physics simulation based on bicycle model
// Following detailed physics breakdown with proper coordinate systems
...
class InputHandler {
  ...
  setupButtonEvents() {
    ...
    const addButtonEvents = (button, buttonKey) => {
      if (!button) return;
  ...
  getControls() {
    return {
      throttle:
        this.keys[CONFIG.input.throttleKey] || this.buttons.throttle ? 1 : 0,
      brake: this.keys[CONFIG.input.brakeKey] || this.buttons.brake ? 1 : 0,
      steerLeft:
        this.keys[CONFIG.input.steerLeftKey] || this.buttons.steerLeft ? 1 : 0,
      steerRight:
...
class Car {
  constructor() {
    ...
    this.Fz_f = (CONFIG.car.mass * 9.81 * CONFIG.car.b) / CONFIG.car.wheelbase;
    this.Fz_r = (CONFIG.car.mass * 9.81 * CONFIG.car.a) / CONFIG.car.wheelbase;
  ...
  update(dt, controls) {
    ...
    const now = Date.now();
    this.trailMarks = this.trailMarks.filter((mark) => now - mark.time < 4000);
  }

  updatePhysicsStep(dt, controls) {
  ...
  processSteeringInput(controls, dt) {
    const config = CONFIG.car;
    ...
    this.steeringAngle +=
      Math.sign(deltaError) * Math.min(Math.abs(deltaError), steerRate * dt);
  ...
  calculateAckermannSteering() {
    ...
    let delta_inner = Math.atan(config.wheelbase / Math.max(innerRadius, 0.1));
    ...
    if (this.steeringAngle > 0) {
      ...
      let innerWheelAngle =
        this.steeringAngle *
        (1 + config.ackermannRatio * (delta_inner / this.steeringAngle - 1));
      let outerWheelAngle =
        this.steeringAngle *
        (1 + config.ackermannRatio * (delta_outer / this.steeringAngle - 1));
  ...
  calculateTireForces() {
    ...
    this.Fy_f = clamp(this.Fy_f, -availableFrontLateral, availableFrontLateral);
    this.Fy_r = clamp(this.Fy_r, -availableRearLateral, availableRearLateral);
  }
...
// Start the aggressive gyro physics game
window.onload = () => {
  new Game();
};
