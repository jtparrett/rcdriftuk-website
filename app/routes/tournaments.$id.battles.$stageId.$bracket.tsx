...
export const loader = async (args: LoaderFunctionArgs) => {
  ...
  const stageId = z.string().uuid().parse(args.params.stageId);
  const bracket = z
  ...
  const stage = await prisma.tournamentBattleStages.findFirst({
    where: { id: stageId, tournamentId: id },
    select: { id: true, name: true, format: true, sortOrder: true },
  });

  notFoundInvariant(stage, "Stage not found");
...
const TournamentBattlesPage = () => {
  const { tournament, bracket, stageId, stageFormat, isOwner } =
  ...
  const battleStages = tournament.battleStages;

  return (
    <>
      <HiddenEmbed>
        <Flex flexDir="column" gap={2} mb={2}>
          {battleStages.length > 1 && (
            <TabGroup>
              {battleStages.map((s) => (
                <Tab
                  key={s.id}
                  to={`/tournaments/${tournament.id}/battles/${s.id}/${bracket}`}
                  isActive={s.id === stageId}
                  data-replace="true"
                  replace
                >
                  {s.name}
          ...
          {stageFormat === TournamentsFormat.DOUBLE_ELIMINATION && (
            <TabGroup>
              {Object.values(BattlesBracket).map((sub) => {
                return (
                  <Tab
                    key={sub}
                    to={`/tournaments/${tournament.id}/battles/${stageId}/${sub}`}
